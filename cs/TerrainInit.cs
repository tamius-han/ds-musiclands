...
public class TerrainInit : MonoBehaviour {
  ...
  public static int   MINIMAL_NUMBER_OF_SONGS_FOR_TERRAIN = 500; // if we're trying to make a map with fewer than this amount 
                                                                 // of songs, we just represent songs with cubes
  ...
  public static int   SONG_GROUPING_CHUNK_LEVEL = 4; 
  ...
  void MakeMusicCubes(TerrainTree tt){
    ...
    for(int i = 0; i < songCount.GetLength(0); i++){
      for(int j = 0; j < songCount.GetLength(1); j++){
        if(songCount[i,j] > 0){
          Peak p = new Peak(0, i << SONG_GROUPING_CHUNK_LEVEL, j << SONG_GROUPING_CHUNK_LEVEL);
          GameObject go = Instantiate( cube4,
                                       new Vector3((float)(i << SONG_GROUPING_CHUNK_LEVEL),
                                                   1.0f,
                                                   (float)(j << SONG_GROUPING_CHUNK_LEVEL)),
                                       Quaternion.identity);
          go.transform.localScale = new Vector3(8.0f, 4f*songCount[i,j], 8.0f);
