...
public class CacheOptions : MonoBehaviour {
  ...
  static int fuse_gpmChunkTries = 8;            // will stop trying to fetch songs from chunk after this many failures.
  ...
  public static void DeleteCacheItem(int id){
    ...
    File.Delete(GlobalData.cachedir + "/full/" + id + ".ogg");
  ...
  public static void FetchItemGpm(MusicPoint mp){
...
//         print("attempting to download item " + mp.id);
    ...
    if(provider == "gpm"){
      ...
      foreach(MusicPoint mp in chunk.allSongs){
        ...
        itemStatus = FindItemStatus(mp.id);
        chunk.lastFetched_gpm++;
        
        if( (itemStatus & GPM_NOT_AVAILABLE) != 0 && (itemStatus & GPM_ERROR) == 0)
          continue;
  ...
  public static void FetchNextN(TerrainChunk chunk, int n, string provider){
    ...
    int exitStatus, itemStatus, limit;
    int nonfailed = 0;
    int attempts = 0;
    MusicPoint mp;
    
    if(provider == "gpm"){
      limit = SS.Min(chunk.lastFetched_gpm + n, chunk.allSongs.Count);
      
      for(int i = chunk.lastFetched_gpm; i < limit; i++){
        
        if(nonfailed >= n || attempts++ > fuse_gpmChunkTries){
          return;
        }
        ...
        if( (itemStatus & (GPM_READY | GPM_STREAMING | GPM_CACHED)) != 0 ){
          nonfailed++;
          continue;
        }
